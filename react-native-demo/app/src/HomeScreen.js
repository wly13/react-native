    ...
    return new Promise((resolve, reject) => {
      this.getLongitudeAndLatitude()
        ...
        .then(locationArr => {
          ...
          let BaiduMap_URL =
            'https://api.map.baidu.com/geocoder/v2/?output=json&ak=7UFcr4CLV6Gdkl9aadrnRZnWND6M8ftd&location=' +
            latitude +
            ',' +
            longitude;
          ...
          <View style={style.flex_row}>
            ...
            <View style={style.movie_item}>
              <View style={style.movie_item_title}>
                ...
                <Text style={item.is3D === true ? style.movie_item_3D : ''}>
                  {item.is3D === true ? '3D' : ' '}
                </Text>
                <Text
                  style={item.isIMAX3D === true ? style.movie_item_IMAX : ''}
                >
                  {item.isIMAX3D === true ? 'IMAX' : ''}
                </Text>
              </View>
              <Text style={{marginTop: 5}}>
                {item.r > -1 ? '时光网评分：' + item.r : '该影片暂无评分'}
              </Text>
              <Text style={{marginTop: 1}}>上映时间：{item.year}</Text>
              <Text style={{marginTop: 1}}>影片类型：{item.movieType}</Text>
              <Text numberOfLines={1} style={{marginTop: 1}}>
                主演：{item.actors}
              </Text>
              <Text style={{marginTop: 1}}>
                今天{item.cC}家影院放映{item.NearestShowtimeCount}场
              </Text>
            </View>
      ...
      <View style={{backgroundColor: '#fff', marginBottom: 30}}>
        <View style={style.header}>
          ...
          <View style={{flex: 8, flexDirection: 'row', marginLeft: 80}}>
            ...
            >
              <Text
                style={{lineHeight: 50, borderColor: '#f5f', borderWidth: 1}}
              >
                正在上映
              </Text>
            </TouchableOpacity>
